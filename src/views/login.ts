import { isAuthenticated, connectWallet, getConnectedAddress, clearSession } from "../lib/wallet.ts";
...
export function loginView(): HTMLElement {
  ...
  container.innerHTML = `
    <div class="login-card">
      ...
      <div id="step-signin" ${walletConnected ? '' : 'hidden'}>
        ...
        <p>Sign a message to authenticate with the council platform.</p>
  ...
  container.querySelector("#signin-btn")?.addEventListener("click", async () => {
    ...
    try {
      await authenticate();
      capture("council_login", { publicKey: getConnectedAddress() });
      navigate("/");
    } catch (error) {
