...
async function renderContent(): Promise<HTMLElement> {
  ...
  if (!state.exists) {
    el.innerHTML =
      `<p style="color:var(--text-muted)">Council not found on the platform. <a href="#/">Back</a></p>`;
    return el;
  }
  ...
  const assetBadges = channels.length > 0
    ? channels.map((ch) => {
      ...
      return `<span class="asset-badge">${escapeHtml(ch.assetCode)}${
        isDefault
          ? ""
          : `<button class="asset-remove" data-asset="${
            escapeHtml(ch.assetCode)
          }" data-contract="${
            escapeHtml(ch.channelContractId)
          }" title="Disable ${escapeHtml(ch.assetCode)}">&times;</button>`
      }</span>`;
    }).join("")
    : '<span style="color:var(--text-muted)">No assets enabled</span>';

  el.innerHTML = `
    <div class="council-header"><a href="#/" class="icon-btn" title="Back" style="color:var(--text)"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5"/><path d="M12 19l-7-7 7-7"/></svg></a><span class="inline-edit" data-field="name"><h2>${
    escapeHtml(council.name || "Unnamed Council")
  }</h2><svg class="edit-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></span><span class="inline-edit" data-field="contactEmail"><span style="color:var(--text-muted);font-size:0.75rem">${
    escapeHtml(state.contactEmail || "add email")
  }</span><svg class="edit-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></span><div class="header-icons"><button class="icon-btn copy-contract" data-value="${
    ...
    <div class="inline-edit" data-field="description" style="margin:0.75rem 0"><p style="color:var(--text-muted);font-size:0.85rem;margin:0">${
    escapeHtml(state.description || "add description")
  }</p><svg class="edit-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></div>
      ...
      if (isSelected) {
        ...
        if (isPlatformAuthed()) {
          try {
            ...
            navigate(`/council?id=${encodeURIComponent(councilId)}`, {
              force: true,
            });
          } catch (err) {
  ...
  el.querySelectorAll(".inline-edit").forEach((wrapper) => {
    wrapper.addEventListener("click", (e) => {
      ...
      const placeholder = field === "name"
        ...
        ? "add email"
        : "add description";
      const currentValue = displayEl.textContent === placeholder
        ? ""
      ...
      const input = document.createElement(isTextarea ? "textarea" : "input") as
        | HTMLInputElement
        | HTMLTextAreaElement;
      input.className = `inline-edit-input edit-${
        field === "name"
          ? "title"
          : field === "contactEmail"
          ? "email"
        ...
        if (newValue !== currentValue) {
          ...
          if (isPlatformConfigured() && isPlatformAuthed()) {
            ...
            const nameEl = el.querySelector(
              '.inline-edit[data-field="name"] h2',
            ) as HTMLElement;
            const emailEl = el.querySelector(
              '.inline-edit[data-field="contactEmail"] span',
            ) as HTMLElement;
            const descEl = el.querySelector(
              '.inline-edit[data-field="description"] p',
            ) as HTMLElement;
            const currentName = nameEl?.textContent === "Unnamed Council"
              ? ""
              : (nameEl?.textContent || "");
            const currentEmail = emailEl?.textContent === "add email"
              ? ""
              : (emailEl?.textContent || "");
            const currentDesc = descEl?.textContent === "add description"
              ? ""
              : (descEl?.textContent || "");
            pushMetadata({
              councilId: councilId!,
  ...
  el.querySelector(".copy-invite-link")?.addEventListener("click", () => {
    ...
    navigator.clipboard.writeText(link).then(() => {
      ...
      btn.innerHTML =
        `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--active)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>`;
      setTimeout(() => {
        btn.innerHTML = orig;
      }, 1500);
    });
  ...
  el.querySelectorAll(".asset-remove").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        const ch = platformChannels.find((c) =>
          c.channelContractId === contractId
        );
  ...
  function renderProviderRow(p: ProviderEntry): string {
    const flags = (p.jurisdictions || []).map((code) => {
      const flag = code.toUpperCase().replace(
        /./g,
    ...
    return `
      <tr class="provider-row" data-status="${
      escapeHtml(p.status)
    }" data-request-id="${escapeHtml(p.requestId || "")}" data-pk="${
      escapeHtml(p.publicKey)
        ...
        <td>
          <span class="badge badge-${statusBadgeClass(p.status)}">${
      escapeHtml(p.status)
    }</span>
        </td>
      </tr>
  ...
  function renderProvidersList(entries: ProviderEntry[]) {
    if (entries.length === 0) {
      providersList.innerHTML =
        `<p style="color:var(--text-muted)">No providers yet. Share the invite link to get started.</p>`;
      return;
    }
    ...
    const popup = providersList.querySelector(
      "#provider-action-popup",
    ) as HTMLDivElement;
    let activeRowId: string | null = null;
    let popupLocked = false;

    // Hover to show popup on Requested rows
    providersList.querySelectorAll(".provider-row[data-status='Requested']")
      .forEach((row) => {
    ...
    popup.addEventListener("mouseleave", (e) => {
      ...
      const activeRow = activeRowId
        ? providersList.querySelector(
          `[data-request-id="${CSS.escape(activeRowId)}"]`,
        )
        : null;
    ...
    // Wire popup approve
    const popupApprove = popup.querySelector(
      ".popup-approve",
    ) as HTMLButtonElement;
    const popupReject = popup.querySelector(
      ".popup-reject",
    ) as HTMLButtonElement;
    ...
    popupApprove.addEventListener("click", async () => {
      ...
      try {
        const { buildInvokeContractTx, submitTx } = await import(
          "../lib/stellar.ts"
        );
        ...
        const txXdr = await buildInvokeContractTx(councilId!, "add_provider", [
          providerAddress,
        ], adminAddress);
  ...
  async function loadProviders() {
    ...
    const requestByKey = new Map<
      string,
      import("../lib/platform.ts").JoinRequest
    >();
  ...
  deployBtn.addEventListener("click", async () => {
    ...
    if (!admin) {
      assetErrorEl.textContent = "Connect your wallet first";
      assetErrorEl.hidden = false;
      return;
    }

    const assetCode = (el.querySelector("#new-asset-code") as HTMLInputElement)
      .value.trim();
    const assetIssuer =
      (el.querySelector("#new-asset-issuer") as HTMLInputElement).value.trim();
    if (!assetCode) {
    ...
    if (isPlatformConfigured() && isPlatformAuthed()) {
      try {
        ...
        if (
          active.find((ch) =>
            ch.assetCode.toUpperCase() === assetCode.toUpperCase()
          )
        ) {
          modal.hidden = true;
          navigate(`/council?id=${encodeURIComponent(councilId)}`, {
      ...
      if (!("error" in sim && sim.error)) {
        ...
        if (isPlatformAuthed()) {
          ...
          await registerChannel(councilId!, {
            channelContractId: derivedAddress,
            assetCode,
        ...
        capture("council_asset_found", {
          assetCode,
          channelId: derivedAddress,
        });
        modal.hidden = true;
        navigate(`/council?id=${encodeURIComponent(councilId)}`, {
          force: true,
    ...
    try {
      ...
      if (isPlatformAuthed()) {
        try {
          await registerChannel(councilId!, {
            channelContractId: newChannelId,
            assetCode,
            assetContractId,
            issuerAddress: assetIssuer,
            label: `${assetCode} Privacy Channel`,
