...
const CONFIG_TS = await Deno.readTextFile(
  new URL("../src/lib/config.ts", HERE),
);
...
//
// mainnet.friendbotUrl: friendbot doesn't exist on mainnet; the source-side
// default is a testnet URL that's never called when STELLAR_NETWORK=mainnet.
const DEFAULT_OK: Record<"testnet" | "mainnet", Set<string>> = {
  testnet: new Set(),
  mainnet: new Set(["friendbotUrl"]),
};
