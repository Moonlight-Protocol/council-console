...
async function renderContent(): Promise<HTMLElement> {
    ...
    el.querySelector("#create-btn")?.addEventListener("click", () => {
      (() => {
        for (const key of Object.keys(sessionStorage)) {
          if (key.startsWith("onboarding_")) sessionStorage.removeItem(key);
        }
      })();
      navigate("/create-council/metadata");
    });
    el.querySelector("#import-btn")?.addEventListener(
      "click",
      () => navigate("/recover-council"),
    );
    return el;
  }
  ...
  const states: Array<
    {
      councilId: string;
      name: string;
      state: Awaited<ReturnType<typeof fetchCouncilState>>;
    }
  > = [];
  const BATCH_SIZE = 5;
  ...
  const rows = states.map(({ councilId, state }) => {
    ...
    const badges = assets.map((a) =>
      `<span class="asset-badge" style="padding:0.2rem 0.5rem;font-size:0.75rem">${
        escapeHtml(a)
      }</span>`
    ).join("");
    const flags = state.jurisdictions.map((j) => {
      ...
      return `<span title="${
        escapeHtml(j.countryCode)
      }" style="font-size:1.1rem">${flag}</span>`;
    }).join(" ");

    return `
      <tr>
        <td><a href="#/council?id=${
      encodeURIComponent(councilId)
    }" style="color:var(--text);text-decoration:none">${
      escapeHtml(state.name || "Unnamed")
    }</a></td>
        <td class="text-left">${
      flags || '<span style="color:var(--text-muted)">--</span>'
    }</td>
        <td>${badges || '<span style="color:var(--text-muted)">--</span>'}</td>
        <td class="text-right" style="white-space:nowrap">
          <span class="icon-btn" title="${state.providers.length} provider${
      state.providers.length !== 1 ? "s" : ""
    }" style="cursor:default"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 12h4"/><path d="M10 8h4"/><path d="M14 21v-3a2 2 0 0 0-4 0v3"/><path d="M6 10H4a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-2"/><path d="M6 21V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v16"/></svg></span>
          <button class="icon-btn copy-invite" data-link="${
      escapeHtml(inviteLink)
    }" title="Copy invite link"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></button>
          <button class="icon-btn delete-council" data-id="${
      escapeHtml(councilId)
    }" title="Delete council"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg></button>
        </td>
      </tr>
    `;
  }).join("");
  ...
  el.querySelector("#new-council-btn")?.addEventListener("click", () => {
    (() => {
      for (const key of Object.keys(sessionStorage)) {
        if (key.startsWith("onboarding_")) sessionStorage.removeItem(key);
      }
  ...
  el.querySelectorAll(".delete-council").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const msg = providerCount > 0
        ? `Delete this council? This will remove ${providerCount} provider${
          providerCount !== 1 ? "s" : ""
        } on-chain and delete all data from the platform.`
      ...
      try {
        ...
        if (providerCount > 0 && councilState) {
          const { sdk: getSdk, getRpcServer } = await import(
            "../lib/stellar.ts"
          );
          ...
          for (
            let batchStart = 0;
            batchStart < providers.length;
            batchStart += OPS_PER_TX
          ) {
            ...
            for (const p of batch) {
              builder.addOperation(
                contract.call(
                  "remove_provider",
                  stellar.nativeToScVal(
                    stellar.Address.fromString(p.publicKey),
                    { type: "address" },
                  ),
                ),
              );
            ...
            const signed = stellar.TransactionBuilder.fromXDR(
              signedXdr,
            ...
            for (let i = 0; i < 30; i++) {
              ...
              if (status.status === "FAILED") {
                throw new Error("remove_provider transaction failed");
              }
              await new Promise((r) => setTimeout(r, 2000));
            }
            if (!confirmed) {
              throw new Error(
                "remove_provider transaction not confirmed after polling — aborting delete",
              );
            }
          }
        }
