...
async function writeHealthJson(version: string): Promise<void> {
  const health = { status: "ok", service: "council-console", version };
  await Deno.writeTextFile(HEALTH_OUT, JSON.stringify(health) + "\n");
  console.log(`Built public/health.json (council-console ${version})`);
}
...
async function buildStyles(): Promise<void> {
  ...
  parts.push(`/* council-console app-styles */\n${appStyles}`);
